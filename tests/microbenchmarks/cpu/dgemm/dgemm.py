# Copyright 2016-2022 Swiss National Supercomputing Centre (CSCS/ETH Zurich)
# ReFrame Project Developers. See the top-level LICENSE file for details.
#
# SPDX-License-Identifier: BSD-3-Clause

import reframe as rfm
import reframe.utility.sanity as sn
import contextlib


@rfm.simple_test
class DGEMMTestCycloneCPU(rfm.RegressionTest):
    descr = "DGEMM performance test"
    sourcepath = "dgemm.c"

    # TODO: Generalize this to use a list with all available nodes
    node_id = parameter(["cn02", "cn03", "cn04"])

    # the perf patterns are automaticaly generated inside sanity
    perf_patterns = {}
    valid_systems = ["cyclone:cpu"]
    # valid_prog_environs = ['PrgEnv-gnu-nompi-nocuda', 'PrgEnv-intel-nompi']
    valid_prog_environs = ["PrgEnv-intel-nompi"]

    modules = ["OpenBLAS/0.3.20-GCC-11.3.0", "imkl/2022.2.1"]

    exclusive_access = True
    num_tasks = 1
    use_multithreading = False
    executable_opts = ["6144", "12288", "3072"]
    build_system = "SingleSource"

    flops_reference = {
        "PrgEnv-gnu-nompi-nocuda": {
            "cyclone:cpu": {"flops": (2250, -0.05, None, "Gflop/s")},
        },
        "PrgEnv-intel": {
            "cyclone:cpu": {"flops": (2350, -0.05, None, "Gflop/s")},
        },
    }

    cores = variable(
        dict,
        value={
            "cyclone:cpu": 40,
        },
    )

    maintainers = ["CS"]
    tags = {"benchmark", "diagnostic"}

    @run_after("setup")
    def setup_reference(self):
        envname = self.current_environ.name

        try:
            self.reference = self.flops_reference[envname]
        except KeyError:
            self.reference = self.flops_reference["PrgEnv-gnu-nompi-nocuda"]

    @run_before("compile")
    def set_compile_flags(self):
        # FIXME: To avoid using the `startswith()` we can now use the
        # environment `extras` to encode the compiler family, if properly
        # configured in the environment definitions.

        self.build_system.cflags = ["-O3"]
        if self.current_environ.name.startswith("PrgEnv-gnu"):
            self.build_system.cflags += ["-fopenmp"]
            self.build_system.cflags += ["-I$EBROOTOPENBLAS/include"]
            self.build_system.ldflags = [
                "-L$EBROOTOPENBLAS/lib",
                "-lopenblas",
                "-lpthread",
                "-lgfortran",
            ]
        elif self.current_environ.name.startswith("PrgEnv-intel"):
            self.build_system.cppflags = ["-DMKL_ILP64", "-I${MKLROOT}/include"]
            self.build_system.cflags = ["-qopenmp"]
            self.build_system.ldflags = [
                "-L${MKLROOT}/lib/intel64",
                "-lmkl_intel_lp64",
                "-lmkl_intel_thread",
                "-lmkl_core",
                "-liomp5",
                "-lpthread",
                "-lm",
                "-ldl",
            ]

    @run_before("run")
    def prepare_run(self):
        self.num_cpus_per_task = self.cores.get(self.current_partition.fullname, 1)
        self.env_vars = {
            "OMP_NUM_THREADS": str(self.num_cpus_per_task),
            "OMP_BIND": "cores",
            "OMP_PROC_BIND": "spread",
            "OMP_SCHEDULE": "static",
            "KMP_AFFINITY": "granularity=fine,compact",
        }
        self.job.options = [f"--nodelist={self.node_id}"]

    @sanity_function
    def validate(self):
        hostname = self.node_id
        all_tested_nodes = sn.evaluate(
            sn.extractall(
                r"(?P<hostname>\S+):\s+Time for \d+ DGEMM operations",
                self.stdout,
                "hostname",
            )
        )
        num_tested_nodes = len(all_tested_nodes)
        failure_msg = "Requested %s node(s), but found %s node(s)" % (
            self.job.num_tasks,
            num_tested_nodes,
        )
        sn.evaluate(sn.assert_eq(num_tested_nodes, self.job.num_tasks, msg=failure_msg))

        self.perf_patterns = {
            "flops": sn.extractsingle(
                rf"{hostname}:\s+Avg\. performance\s+:\s+(?P<gflops>\S+)" rf"\sGflop/s",
                self.stdout,
                "gflops",
                float,
            )
        }

        return True
