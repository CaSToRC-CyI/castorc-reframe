...
int main(int argc, const char *argv[]) {
  ...
  while (end == 0) {
    ...
    if (end == 0) {
      ...
      color = (comm_rank < j);
      if (color == 0) {
        if (MPI_Comm_split(MPI_COMM_WORLD, MPI_UNDEFINED, comm_rank,
                           &red_comm) != 0) {
          fprintf(stderr, "MPI_Comm_split() failed\n");
          exit(1);
        }
      } else {
        if (MPI_Comm_split(MPI_COMM_WORLD, color, comm_rank, &red_comm) != 0) {
          fprintf(stderr, "MPI_Comm_split() failed\n");
          exit(1);
        }
        /* cartesian grid communicator */
        if (MPI_Cart_create(red_comm, ndims, dim_size, periods, reorder,
                            &cart_comm) != 0) {
        ...
        for (j = 0; j < NCALLS; j++) {
          for (i = 0; i < ndims; i++) {
            ...
            if (MPI_Cart_shift(cart_comm, i, 1, &rank_source, &rank_dest) !=
                0) {
              fprintf(stderr, "MPI_Cart_shift() failed\n");
              exit(1);
            }
            if (MPI_Irecv(recvbuf + i * 2 * halosize[i] * sizeof(char),
                          halosize[i], MPI_CHAR, rank_source, 1, cart_comm,
                          request + i * 2) != 0) {
              fprintf(stderr, "MPI_Irecv() failed\n");
              exit(1);
            }
            if (MPI_Irecv(recvbuf + (i * 2 + 1) * halosize[i] * sizeof(char),
                          halosize[i], MPI_CHAR, rank_dest, 1, cart_comm,
                          request + i * 2 + 1) != 0) {
              fprintf(stderr, "MPI_Irecv() failed\n");
              exit(1);
            }
          ...
          for (i = 0; i < ndims; i++) {
            ...
            if (MPI_Isend(sendbuf + i * 2 * halosize[i] * sizeof(char),
                          halosize[i], MPI_CHAR, rank_source, 1, cart_comm,
                          request + i * 2 + ndims * 2) != 0) {
              fprintf(stderr, "MPI_Irecv() failed\n");
              exit(1);
            }
            if (MPI_Isend(sendbuf + (i * 2 + 1) * halosize[i] * sizeof(char),
                          halosize[i], MPI_CHAR, rank_dest, 1, cart_comm,
                          request + i * 2 + 1 + ndims * 2) != 0) {
              fprintf(stderr, "MPI_Irecv() failed\n");
              exit(1);
            }
          }
        ...
        if (MPI_Reduce(&elapsed_time, &deltatmax, 1, MPI_DOUBLE, MPI_MAX, 0,
                       cart_comm) != 0) {
          fprintf(stderr, "MPI_Reduce() failed\n");
          exit(1);
        }
