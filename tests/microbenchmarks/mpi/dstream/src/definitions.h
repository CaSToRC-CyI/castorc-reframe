...
typedef enum { none, individual, collective } persist_state;
...
typedef enum { copy, scale, add, triad } benchmark_type;

int stream_memory_task(benchmark_results *b_results, communicator world_comm,
                       communicator node_comm, int *array_size, int cache_size,
                       int repeats);
...
void save_results(char *filename, benchmark_results *all_node_results,
                  int array_size, communicator world_comm,
                  communicator node_comm, communicator root_comm);
