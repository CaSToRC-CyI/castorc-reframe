 ...
 *               If the processor and memory are capable of 10 GB/s, this is 2
 *GB in 200 msec. This means the each array must be at least 1 GB.
 */
...
int stream_memory_task(benchmark_results *b_results, communicator world_comm,
  ...
  *array_size = (cache_size * 4) / node_comm.size;
  ...
  if (world_comm.rank == ROOT) {
    ...
    printf(
        "Total memory required per process = %.1f MiB (= %.1f GiB).\n",
    ...
    printf("Total memory required per node = %.1f MiB (= %.1f GiB).\n",
           node_comm.size * (3.0 * BytesPerWord) *
               ((double)*array_size / 1024.0 / 1024.),
           node_comm.size * (3.0 * BytesPerWord) *
               ((double)*array_size / 1024.0 / 1024. / 1024.));
...
#pragma omp parallel for
  ...
  scalar = 3.0;
  for (k = 0; k < repeats; k++) {
    // Add in a barrier synchronisation to ensure all processes on a node are
    // undertaking the benchmark at the same time. This ensures the node level
    // results are fair as all operations are synchronised on the node.
