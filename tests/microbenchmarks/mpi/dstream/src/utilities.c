...
/*int name_to_colour(const char *name){
  ...
  for(p=name; *p ; p++){
    ...
    if (small_multiplier > 0 && (res * small_multiplier) > (INT_MAX /
small_multiplier)) {
  ...
  // If we have ended up with a negative result, invert it to make it positive
because
  // the functionality (in MPI) that we will use this for requires the int to be
positive. if( res < 0 ){ res = -res;
  }
  return res;
}*/
...
// TODO: This might be general enough to provide the functionality for any
// system regardless of processor type given we aren't worried about
// thread/process migration. Test on Intel systems and see if we can get rid of
// the architecture specificity of the code.
unsigned long get_processor_and_core(int *chip, int *core) {
  return syscall(SYS_getcpu, core, chip, NULL);
}
// TODO: Add in AMD function
#else
// If we're not on an ARM processor assume we're on an intel processor and use
