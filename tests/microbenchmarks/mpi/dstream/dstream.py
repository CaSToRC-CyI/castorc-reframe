...
class StreamTest(rfm.RegressionTest):
    def __init__(self):
        ...
        self.sanity_patterns = sn.assert_found(r'Node Triad',
                                               self.stdout)
        self.perf_patterns = {
                'Copy': sn.extractsingle(r'Node Copy:(\s+\S+:){0}\s+(?P<val>\S+):',
                                     self.stdout, 'val', float, item=-1),
                'Scale': sn.extractsingle(r'Node Scale:(\s+\S+:){0}\s+(?P<val>\S+):',
                                     self.stdout, 'val', float, item=-1),
                'Add': sn.extractsingle(r'Node Add:(\s+\S+:){0}\s+(?P<val>\S+):',
                                     self.stdout, 'val', float, item=-1),
                'Triad': sn.extractsingle(r'Node Triad:(\s+\S+:){0}\s+(?P<val>\S+):',
                                     self.stdout, 'val', float, item=-1),
        }
        self.reference = {
            'cyclone:cpu': {
